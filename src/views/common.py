...
def render_stats_toolbar(
    subset: pd.DataFrame, parameter: str, key_prefix: str, display_label: str | None = None
) -> None:
    """Min/mean/max/mode/total cards for one parameter's filtered data.

    ``display_label`` overrides the card titles (used when ``parameter``
    is a composite's "primary" parameter, but the cards should read the
    composite's own name instead, e.g. "Temperature" not "Temperature Air
    Mean 2m").
    """
...
def render_parameter_and_subset(
    ...
    dropdown entry -- e.g. "Temperature" for the 2m variants + ground
    frost, "Precipitation" for height + form. When a composite is
    selected, the returned ``subset`` contains every one of its
    components' rows (so a caller like time_series.py can build one chart
    per component); the Key Figures toolbar underneath is computed off
    just that composite's "primary" parameter, labeled with the
